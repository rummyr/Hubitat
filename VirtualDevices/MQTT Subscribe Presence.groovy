...
preferences {
    section("URIs") {
        ...
        input name: "mqttUser", type: "text", title: "User for connecting to MQTT broker", required: true, defaultValue: "iot"
        input name: "mqttPassword", type: "password", title: "Password for connecting to MQTT broker", required: true
...
def connect() {
    ...
    try {
        trace "full args: ('tcp://$broker:$port', '${device.displayName}', '$mqttUser', '$mqttPassword')"
    	interfaces.mqtt.connect("tcp://$broker:$port", device.displayName, mqttUser, mqttPassword)
...
def updated() {
    ...
    info "user is $mqttUser"
    trace "password is $mqttPassword"
    reconnect();
...
/** originally called periodically, sends a "test" message to MQTT, should be received */
def unusued_checkMQTT_bySending() {
